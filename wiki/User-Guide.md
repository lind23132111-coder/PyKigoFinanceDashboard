...
![Expenses V2.2 Desktop](images/expenses_v2.2_desktop.png)
*V2.2 全新配置：靈活的時間過濾與強化的行動端體驗。*


- **靈活過濾與時間觀測 (Flexible Filtering)**:
  - **切換模式**: 您可以在「月」、「季」、「年」之間快速切換。系統會自動計算該區間的總支出、日均支出，並對比「前一週期」的增長百分比。
  - **專案標籤 (Project Tabs)**: 第一排標籤可切換全部、日常或特定專案。點擊後，下方所有圓餅圖與統計數據會即時過濾，讓您專注於特定預算的執行狀況。
...
- **工作區域化管理 (Transaction Workspace)**:
  ...
  - **易讀性優化**: 針對小螢幕大幅調大了商家名稱與金額的字體，並優化了標籤顯示（如專案、支付人）。
  - **固定關閉鍵**: 關閉按紐 (X) 採絕對定位，即使標題換行也不會位移，操作更穩健。
