...
- **AI 智慧匯入中心**:
  ...
  - **智慧去重**: AI 會自動比對「日期、金額、商店」，若偵測到重複會顯示警告。
  ![Duplicate Detection](images/expenses_uncheck_ultra.png)
  - **批次處理**: 您可以使用「勾選重複項」功能快速批次刪除，大幅降低核對負擔。
  - **自動化標籤**: AI 會自動判斷消費類別，並根據消費內容建議合適的帳本專案。

- **分帳淨額結算模組**:
  ![Split Settlement](images/expenses_AA_ultra.png)
  - **墊付追蹤**: 顯示目前 PY 與 Kigo 的總墊付金額。系統採用「淨負債」邏輯，直接計算出誰應支付給誰。
  - **部分結算支援**: 點擊「執行結算」可以紀錄全額或部分結清，結算金額會自動從負債餘額中扣除。
  - **歷史對帳**: 點擊結算卡片右上角的「歷史」圖示，可查看過往所有結算軌跡與備註。

- **高效排版設計**:
  - **可收合 AI Inbox**: 「待核對清單」預設為收合，點擊標題可展開進行批次確認。
  - **統計報表優先**: 頁面頂部優先顯示分析報表與結算狀態，方便快速瀏覽。
