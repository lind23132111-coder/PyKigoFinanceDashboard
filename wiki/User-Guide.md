...
![Expenses V2.1 Final](images/expenses_v2.1_final.png)
*V2.1 全新配置：分層過濾系統與工作區域化排版。*


- **分層過濾系統 (Two-Row Filter)**:
  - **第一排 (專案標籤)**: 快速切換「全部」、「日常」或特定的「財務目標（如：裝修、旅遊）」。點擊標籤後，下方所有統計卡片與圓餅圖都會即時過濾。
  - **第二排 (時間篩選)**: 提供月份選擇器。當您想查看過往特定區間時，資訊呈現更加大氣且易於點選。
...
- **工作區域化管理 (Transaction Workspace)**:
  ...
  - **批次處理**: 進入中心後可選取多筆資料進行「批次核對」或「批次刪除」。
  - **全歷史過濾 (Quick Lookup)**: 支援跨月關鍵字搜尋與精確的「開始/結束」日期選擇器。
