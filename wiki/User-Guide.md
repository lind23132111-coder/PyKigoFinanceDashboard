...
## 5. 行動版介面優化 (Mobile Optimization)
為了讓您在手機上也能流暢更新資產，我們針對行動裝置進行了深度優化：

- **底部導覽選單**：在手機版中，原本頂部的導覽列會轉換為底部的固定式選單。您可以輕鬆地使用大拇指在「首頁」、「目標」、「結算」、「報告」與「策略」之間切換。
- **置頂篩選功能**：在 Dashboard 使用互動式篩選時，篩選顯示列會自動置頂在畫面最上方，即使您正在下滑查看後方的圖表，也能隨時點擊「清除」來回到完整視角。
- **結算頁面加速**：Wizard 頁面的篩選按鈕（如：切換 PY/Kigo 帳戶）與輸入框均經過加大處理，讓您在行動中也能快速輸入銀行餘額。

---
**Happy Financial Planning!**
### 📉 投資策略與規劃 (Strategy & Planning) - [💡 Beta / 功能測試]
在 **"Strategy"** 頁面中，您可以進行更長遠的財務佈局：

1. **資產選取 (Asset Selector)**：
   - **電腦版**：左側配有完整的資產清單與搜尋功能。
   - **手機版**：圖表上方配有快速下拉選單，節省空間。
2. **TradingView 深度分析**：
   - 查看標的即時 K 線圖與技術指標。
   - 手機版提供加高後的技術線圖 (**600px**)，方便精確畫線與觀察。
3. **戰術筆記 (Tactical Notes)**：
   - 即時紀錄對該標的的看法、目標買賣價。
   - 手機版筆記常置於圖表下方，下滑即可進入編輯。
4. **預測功能 (WIP)**：
   - **資產再平衡**：設定理想比例，系統提醒是否偏離目標。
   - **股息滾雪球**：預測未來 10 年股息與複利效果。
