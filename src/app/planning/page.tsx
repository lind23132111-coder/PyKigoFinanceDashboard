...
export default function PlanningPage() {
    ...
    if (loading || !planningData) return <div className="p-8 animate-pulse text-slate-400">{t('planning.loadingPlan')}</div>;
        ...
        <div className="max-w-[1600px] mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-12">
            ...
            <div className="flex justify-between items-end">
                <div>
                    <div className="flex items-center gap-3">
                        <h1 className="text-xl sm:text-3xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                            ...
                            {t('planning.title')} <span className="text-brand-600 text-lg">{t('planning.titleEnSuffix')}</span>
                        </h1>
                ...
                <div className="bg-emerald-50 border border-emerald-100 px-5 py-2.5 rounded-2xl flex items-center gap-4 shadow-sm">
                    ...
                    <div>
                        <div className="text-[10px] text-emerald-600 font-black uppercase tracking-widest">{planningData.userGoal?.goal_name || t('planning.goalProgressLabel')}</div>
                        <div className="text-lg font-black text-slate-700 leading-tight">
                            {achievementPercent}% <span className="text-xs font-bold text-slate-400">{t('planning.achievedLabel')}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="glass p-8 rounded-3xl border border-slate-200/60 shadow-lg space-y-6 bg-gradient-to-br from-white/80 to-slate-50/50 overflow-visible relative z-10">
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-xl font-black text-slate-800 flex items-center gap-3">
                                ...
                                {t('planning.rebalancingTitle')}
                            </h2>
                            <p className="text-xs font-bold text-slate-400 mt-1">{t('planning.rebalancingSubtitle')}</p>
                        </div>
                        {rebalanceNeeded && (
                            <div className="bg-amber-100 text-amber-600 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest animate-pulse">
                                {t('planning.rebalanceRecommended')}
                            </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-8 h-44">
                        <div className="flex flex-col items-center">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">{t('planning.targetAllocation')}</span>
                        ...
                        <div className="flex flex-col items-center">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">{t('planning.actualAllocation')}</span>
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-slate-100">
                        {rebalanceItems.map((item: any) => {
                            const getDefinition = (cat: string) => {
                                if (cat.includes("核心") || cat.toLowerCase().includes("core")) return "Core index assets seeking market returns with lower volatility.";
                                if (cat.includes("成長") || cat.toLowerCase().includes("growth")) return "High potential tech or momentum stocks for alpha.";
                                if (cat.includes("定存") || cat.toLowerCase().includes("deposit")) return "Low volatility dividend or cash flow assets.";
                                return "Investment strategy guidelines for this category.";
                            };
                                ...
                                <div key={item.category} className="flex flex-col bg-white p-2.5 rounded-xl border border-slate-100 shadow-sm relative group/card cursor-help overflow-visible">
                                    ...
                                    <div className="flex items-center justify-between mt-1 pl-1">
                                        ...
                                        <span className={`text-[8px] font-black px-1.5 py-0.5 rounded-md ${item.diff > 2 ? 'bg-rose-100 text-rose-600' :
                                            ...
                                            {item.diff > 2 ? t('planning.sellLabel') : item.diff < -2 ? t('planning.buyLabel') : t('planning.holdLabel')}
                                        </span>
                                    </div>
                                </div>
                            );
                ...
                <div className="glass p-8 rounded-3xl border border-slate-200/60 shadow-lg space-y-6 bg-gradient-to-br from-white/80 to-emerald-50/30">
                    <div className="flex justify-between items-center">
                        <div>
                            <h2 className="text-xl font-black text-slate-800 flex items-center gap-3 group relative">
                                ...
                                {t('planning.dividendSnowballTitle')}
                                <div className="relative group/tooltip">
                                    ...
                                    <div className="absolute left-0 top-full mt-3 w-64 p-4 bg-slate-900 text-white text-[11px] rounded-2xl shadow-2xl opacity-0 group-hover/tooltip:opacity-100 pointer-events-none transition-all duration-300 z-50 transition-translate transform -translate-y-2 group-hover/tooltip:translate-y-0">
                                        <div className="font-black text-brand-400 mb-2 uppercase tracking-widest border-b border-white/10 pb-1">{t('planning.dataCalcExplanation')}</div>
                                        <p className="leading-relaxed font-medium mb-2">
                                            {t('planning.dataCalcLine1')}
                                        </p>
                                        <ul className="space-y-1.5 list-disc pl-3 text-white/70">
                                            <li>{t('planning.dataCalcLine2')}</li>
                                            <li>{t('planning.dataCalcLine3')}</li>
                                            <li>{t('planning.dataCalcLine4')}</li>
                                        </ul>
                            ...
                            <p className="text-xs font-bold text-slate-400 mt-1">{t('planning.dividendSnowballSubtitle')}</p>
                        </div>
                    </div>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={planningData.dividendProjections}>
                                ...
                                <Tooltip
                                    ...
                                    formatter={(value) => [`$${Number(value).toLocaleString()}`, 'Dividend']}
                                />
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center gap-3">
                            ...
                            <div>
                                <div className="text-[9px] text-slate-400 font-black uppercase tracking-widest">{t('planning.currentAnnualIncome')}</div>
                        ...
                        <div className="p-4 bg-brand-500 rounded-2xl border border-brand-600 shadow-lg shadow-brand-200 flex items-center gap-3">
                            ...
                            <div>
                                <div className="text-[9px] text-white/70 font-black uppercase tracking-widest">
                                    {t('planning.projectedAnnualIncome', { year: planningData.dividendProjections[planningData.dividendProjections.length - 1]?.year || '' })}
                                </div>
                        ...
                        <div className="flex items-center gap-4 flex-1">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <h2 className="text-[10px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                    {t('planning.liveAnalysisTitle')} <span className="text-brand-600 font-black">{selectedStock || t('planning.selectStockPrompt')}</span>
                                </h2>
                            </div>
                        </div>
                        ...
                        {selectedAsset && (
                            <div className="flex items-center flex-wrap gap-2 lg:gap-4 w-full lg:w-auto justify-end">
                                <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-slate-100 shadow-sm">
                                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{t('planning.strategyCategoryLabel')}</span>
                                    <select
                                    ...
                                    >
                                        <option value="">{t('planning.uncategorized')}</option>
                                ...
                                <div className="flex items-center gap-2">
                                    {selectedAsset.recommendedCategory && selectedAsset.currentCategory !== selectedAsset.recommendedCategory && (
                                        ...
                                        >
                                            ...
                                            {t('planning.adoptRecommendation')}
                                        </button>
                                    )}
                    ...
                    <div className="flex-1 relative h-[600px] lg:h-full min-h-[600px] lg:min-h-[500px]">
                        {selectedStock ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-3 p-8 text-center bg-slate-50/30">
                                ...
                                <div>
                                    <p className="font-black text-sm text-slate-500">{t('planning.waitingSelectionTitle')}</p>
                                    <p className="text-[10px] font-bold text-slate-400">{t('planning.waitingSelectionDesc')}</p>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="col-span-12 lg:col-span-3 glass rounded-3xl border border-slate-200/60 shadow-xl p-6 flex flex-col bg-white/40 backdrop-blur-xl overflow-hidden transition-all duration-300">
                    <div className="flex items-center justify-between mb-5">
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <h2 className="text-sm font-black text-slate-800 leading-tight">{t('planning.tacticalNotesHeader')}</h2>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">{selectedStock || t('planning.notesRemarks')}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto pr-1 scrollbar-thin">
                        {selectedStock ? (
                            ...
                            <div className="h-full flex flex-col items-center justify-center text-slate-300 italic text-xs space-y-2 opacity-60">
                                ...
                                <p>{t('planning.waitingSelectionTitle')}...</p>
                            </div>
                        )}
                    </div>
                </div>
