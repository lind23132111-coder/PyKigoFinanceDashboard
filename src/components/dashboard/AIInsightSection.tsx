...
export const AIInsightSection: React.FC<AIInsightSectionProps> = ({
    ...
    const { t } = useLanguage();
    const isLoading = latestSummary.includes('正在為您產生') || latestSummary.includes('載入最新的財務數據') || latestSummary.includes('Generating') || latestSummary.includes('Loading');
        ...
        <div className="bg-gradient-to-r from-brand-50 to-indigo-50 border border-brand-100 rounded-2xl p-5 flex items-start gap-4">
            ...
            <div className="flex-1 w-full">
                ...
                <div className="mt-3 flex gap-2 relative">
                    ...
                    >
                        {isRegenerating ? <span className="animate-pulse text-xs">{t('dashboard.generatingBtn')}</span> : <><Send className="w-3 h-3" /> {t('dashboard.regenerateBtn')}</>}
                    </button>
                </div>
            </div>
        </div>
